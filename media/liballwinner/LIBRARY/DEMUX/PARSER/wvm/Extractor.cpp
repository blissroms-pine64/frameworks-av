...
#endif
#include "Extractor.h"
extern "C" int SecureMemAdapterCopy(void *dest, void *src, size_t n);
extern "C" void* SecureMemAdapterGetPhysicAddressCpu(void *virt);
extern "C" void* SecureMemAdapterGetVirtualAddressCpu(void *phy);
extern "C" int SecureMemAdapterWrite(void *src, void *dest, size_t n);
extern "C" int SecureMemAdapterDump(void *ptr, size_t size);
...
status_t Extractor::readSampleData(uint8_t *buf, size_t length,
    ...
    if(secure) {
        ...
        src = (uint8_t *)src;
        #else
        src = (uint8_t *)SecureMemAdapterGetVirtualAddressCpu(src);
        #endif
#endif
    }
	if(info->mSample->range_length() > length) {
		if(!secure) {
			...
			SecureMemAdapterCopy(buf, src, length);
			SecureMemAdapterCopy(extraBuf, src + length, info->mSample->range_length() - length);
		}
	} else {
		if(!secure) {
			memcpy(buf, src, info->mSample->range_length());
		} else {
			SecureMemAdapterCopy(buf, src, info->mSample->range_length());
		}
	}
    if (info->mTrackFlags & kIsVorbis) {
        int32_t numPageSamples;
        ...
        if(tailSize > 0) {
        	if((size_t)tailSize >= sizeof(numPageSamples)) {
        		...
        		if(!secure) {
        			memcpy(buf + info->mSample->range_length(), pageSamples, sizeof(numPageSamples));
        		} else {
        			SecureMemAdapterWrite(pageSamples, buf + info->mSample->range_length(), sizeof(numPageSamples));
        		}
        	} else {
        		...
        		if(!secure) {
        			...
        			SecureMemAdapterWrite(pageSamples, buf + info->mSample->range_length(), tailSize);
        			SecureMemAdapterWrite(pageSamples + tailSize, extraBuf, sizeof(numPageSamples) - tailSize);
        		}
        	}
        } else {
        	//case buf is full.
    		if(!secure) {
    			...
    			SecureMemAdapterWrite(&numPageSamples, extraBuf + info->mSample->range_length() - length,
                        sizeof(numPageSamples));
    		}
        }
    }
...
int32_t Extractor::setBuffers(void *buf, int32_t size)
{
    ...
    #else
    MediaBuffer *mbuf = new MediaBuffer(SecureMemAdapterGetPhysicAddressCpu(buf), size);
    #endif
#else
	//For test software.
