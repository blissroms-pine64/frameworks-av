...
static VIDDEC_CUSTOM_PARAM sVideoEncCustomParams[] =
{
	...
	{"OMX.Topaz.index.param.extended_video", (OMX_INDEXTYPE)VideoEncodeCustomParamextendedVideo},
	{"OMX.aw.index.param.videoSuperFrameConfig", (OMX_INDEXTYPE)VideoEncodeCustomParamextendedVideoSuperframe},
	{"OMX.aw.index.param.videoSVCSkipConfig", (OMX_INDEXTYPE)VideoEncodeCustomParamextendedVideoSVCSkip}
};
...
#endif
    m_sInPortDefType.nBufferSize 					 = (OMX_U32)(m_sOutPortDefType.format.video.nFrameWidth*m_sOutPortDefType.format.video.nFrameHeight*3/2);
...
//for 4k recorder @30fps
    ...
    m_sInPortFormatType.nIndex             = 0x1;
...
//          stack.dump("get_parameter");
	        ...
	        else if (((OMX_PARAM_PORTDEFINITIONTYPE*)(pParamData))->nPortIndex == m_sOutPortDefType.nPortIndex)
	        {
	            ...
	            logv(" width = %d, height = %d", (int)m_sOutPortDefType.format.video.nFrameWidth, (int)m_sOutPortDefType.format.video.nFrameHeight);
	        }
	        else
    	...
    	case OMX_IndexParamVideoPortFormat:
    	{
			...
			OMX_VIDEO_PARAM_PORTFORMATTYPE * param_portformat_type = (OMX_VIDEO_PARAM_PORTFORMATTYPE *)(pParamData);
        ...
        case OMX_IndexParamVideoBitrate:
		{
            ...
            if (((OMX_VIDEO_PARAM_BITRATETYPE*)(pParamData))->nPortIndex == m_sOutPutBitRateType.nPortIndex)
            {
    	...
    	default:
    	{
		    switch((VIDENC_CUSTOM_INDEX)eParamIndex)
	    	{
				...
				case VideoEncodeCustomParamextendedVideoSVCSkip:
		        {
					...
					memcpy(pParamData, &mVideoSVCSkip,  sizeof(VencH264SVCSkip));
					break;
				}
			    default:
				{
...
OMX_ERRORTYPE  omx_venc::set_parameter(OMX_IN OMX_HANDLETYPE pHComp, OMX_IN OMX_INDEXTYPE eParamIndex,  OMX_IN OMX_PTR pParamData)
{
    OMX_ERRORTYPE         eError      = OMX_ErrorNone;
    ...
    switch(eParamIndex)
    {
	    case OMX_IndexParamPortDefinition:
	    {
            ...
            if (((OMX_PARAM_PORTDEFINITIONTYPE *)(pParamData))->nPortIndex == m_sInPortDefType.nPortIndex)
            {
			    ...
			    logd("init_input_port: stride = %d, width = %d, height = %d", (int)m_sInPortDefType.format.video.nStride, (int)m_sInPortDefType.format.video.nFrameWidth, (int)m_sInPortDefType.format.video.nFrameHeight);
			}
	        else if (((OMX_PARAM_PORTDEFINITIONTYPE *)(pParamData))->nPortIndex == m_sOutPortDefType.nPortIndex)
	        {
	        	logv("out, nPortIndex: %d", (int)((OMX_PARAM_PORTDEFINITIONTYPE *)(pParamData))->nBufferCountActual);
	    ...
	    case OMX_IndexParamStandardComponentRole:
	    {
    		...
    		{
    			if(!strncmp((char*)comp_role->cRole, "video_encoder.mjpeg", OMX_MAX_STRINGNAME_SIZE))
    			{
    				strncpy((char*)m_cRole,"video_encoder.mjpeg", OMX_MAX_STRINGNAME_SIZE);
    			}
    			else
		...
		{
			OMX_VIDEO_PARAM_INTRAREFRESHTYPE* pComponentParam = (OMX_VIDEO_PARAM_INTRAREFRESHTYPE*)pParamData;
	    ...
	    default:
	    {
			...
			switch((VIDENC_CUSTOM_INDEX)eParamIndex)
	    	{
				...
				case VideoEncodeCustomParamPrependSPSPPSToIDRFrames:
				{
					...
					m_prependSPSPPSToIDRFrames = ((PrependSPSPPSToIDRFramesParams*)pParamData)->bEnable;
				...
				case VideoEncodeCustomParamextendedVideoSVCSkip:
		        {
					printf("set VideoEncodeCustomParamextendedVideoSVCSkip\n");
					memcpy(&mVideoSVCSkip, pParamData, sizeof(VencH264SVCSkip));
					break;
				}
			    default:
				{
...
OMX_ERRORTYPE  omx_venc::get_config(OMX_IN OMX_HANDLETYPE pHComp, OMX_IN OMX_INDEXTYPE eConfigIndex, OMX_INOUT OMX_PTR pConfigData)
{
	OMX_ERRORTYPE eError = OMX_ErrorNone;
...
OMX_ERRORTYPE omx_venc::set_config(OMX_IN OMX_HANDLETYPE pHComp, OMX_IN OMX_INDEXTYPE eConfigIndex, OMX_IN OMX_PTR pConfigData)
{
		...
		case OMX_IndexConfigVideoBitrate:
		{
			...
			if(pData->nPortIndex == 1)
			{
				if(m_state == OMX_StateExecuting && m_encoder)
				{
					...
					logv("FUNC:%s, LINE:%d , pData->nEncodeBitrate = %d", __FUNCTION__,__LINE__,pData->nEncodeBitrate);
				}
				break;
			}
		}
...
OMX_ERRORTYPE  omx_venc::get_extension_index(OMX_IN OMX_HANDLETYPE pHComp, OMX_IN OMX_STRING pParamName, OMX_OUT OMX_INDEXTYPE* pIndexType)
{
	unsigned int  nIndex;
    ...
    {
        if(strcmp((char *)pParamName, (char *)&(sVideoEncCustomParams[nIndex].cCustomParamName)) == 0)
        {
...
void setSuperFrameCfg(omx_venc* pVenc)
{
	...
	logd("setSuperFrameCfg, pSelf->m_framerate: %d, bitrate: %d", (int)pVenc->m_framerate, (int)pVenc->m_sOutPortDefType.format.video.nBitrate);
	...
	sSuperFrameCfg.nMaxIFrameBits = pVenc->m_sOutPortDefType.format.video.nBitrate/pVenc->m_framerate*5;
	sSuperFrameCfg.nMaxPFrameBits = pVenc->m_sOutPortDefType.format.video.nBitrate/pVenc->m_framerate*10;
	VideoEncSetParameter(pVenc->m_encoder, VENC_IndexParamSuperFrameConfig, &sSuperFrameCfg);
}

void setSVCSkipCfg(omx_venc* pVenc)
{
	logd("setSVCSkipCfg, pSelf->mVideoSVCSkip.uTemporalSVC: %d, .uTemporalSVC: %d\n", (int)pVenc->mVideoSVCSkip.nTemporalSVC, (int)pVenc->mVideoSVCSkip.nSkipFrame);
	VideoEncSetParameter(pVenc->m_encoder, VENC_IndexParamH264SVCSkip, &pVenc->mVideoSVCSkip);
}
...
int openVencDriver(omx_venc* pVenc)
{
	...
	switch(pVenc->m_sInPortFormatType.eColorFormat)
	{
		case OMX_COLOR_FormatYUV420SemiPlanar:
			...
			if(pVenc->mIsFromCts || OPTION_CHIP_1625 == CONFIG_CHIP)
	...
	{
		if(pVenc->m_sInPortFormatType.eColorFormat == OMX_COLOR_FormatAndroidOpaque && pVenc->mIsFromCts)
		{
        ...
        if (FD_ISSET(pSelf->m_cmdpipe[0], &rfds))
        {
	                ...
	                switch ((OMX_STATETYPE)(pCmdData))
	                {
                        ...
                        case OMX_StateIdle:
		                    ...
		                    else
		                    {
			                    ...
			                    while (1)
			                    {
			                    	logv("pSelf->m_sInPortDefType.bPopulated:%d, pSelf->m_sOutPortDefType.bPopulated: %d", pSelf->m_sInPortDefType.bPopulated, pSelf->m_sOutPortDefType.bPopulated);
	        ...
	        else if (cmd == StopPort)
	        {
		        ...
		        while (1)
		        {
		            ...
		            if ((int)pCmdData == -1 &&  !pSelf->m_sInPortDefType.bPopulated && !pSelf->m_sOutPortDefType.bPopulated)
		            {
	        ...
	        else if (cmd == RestartPort)
	        {
		        ...
		        while (1)
		        {
		            ...
		            if (pCmdData == 0x0 && (pSelf->m_state == OMX_StateLoaded || pSelf->m_sInPortDefType.bPopulated))
		            {
		            ...
		            else if (pCmdData == 0x1 && (pSelf->m_state == OMX_StateLoaded || pSelf->m_sOutPortDefType.bPopulated))
		            {
		            ...
		            else if ((int)pCmdData == -1 && (pSelf->m_state == OMX_StateLoaded || (pSelf->m_sInPortDefType.bPopulated && pSelf->m_sOutPortDefType.bPopulated)))
		            {
		            	pSelf->m_Callbacks.EventHandler(&pSelf->mOmxCmp, pSelf->m_pAppData, OMX_EventCmdComplete, OMX_CommandPortEnable, 0x0, NULL);
		            	pSelf->m_Callbacks.EventHandler(&pSelf->mOmxCmp, pSelf->m_pAppData, OMX_EventCmdComplete, OMX_CommandPortEnable, 0x1, NULL);
			            break;
	        ...
	        else if (cmd == FillBuf)
	        {
	        	...
	        	{
	        		pSelf->m_sOutBufList.nSizeOfList++;
	        		OutBufHdr = pSelf->m_sOutBufList.pBufHdrList[pSelf->m_sOutBufList.nWritePos++] = ((OMX_BUFFERHEADERTYPE*) pCmdData);
	        ...
	        else if (cmd == EmptyBuf)
	        {
	        	...
	        	{
	        		pSelf->m_sInBufList.nSizeOfList++;
	        		inBufHdr = pSelf->m_sInBufList.pBufHdrList[pSelf->m_sInBufList.nWritePos++] = ((OMX_BUFFERHEADERTYPE*) pCmdData);
					...
					if(!pSelf->m_useAllocInputBuffer)
					{
						...
						else
						{
							...
							if(buffer_type == kMetadataBufferTypeGrallocSource)
							{
								...
								const Rect rect(pSelf->m_sInPortDefType.format.video.nFrameWidth, pSelf->m_sInPortDefType.format.video.nFrameHeight);
								unsigned char *img;
								void *PhyAddr;
								int res;
		                        ...
		                        {
									int colorFormat;
#if((CONFIG_PRODUCT == OPTION_PRODUCT_PAD) || (CONFIG_CHIP == OPTION_CHIP_1689))
					...
					struct ion_handle *handle_ion;
							...
							else
							{
								...
								else
								{
									...
									sInputBuffer.pAddrPhyC= sInputBuffer.pAddrPhyY + pSelf->m_sInPortDefType.format.video.nStride*pSelf->m_sInPortDefType.format.video.nFrameHeight;
								}
							}
						...
						if(pInBufHdr->nFlags & OMX_BUFFERFLAG_EOS)
						{
							...
							{
								if(pSelf->mVideoExtParams.ui16CropLeft || pSelf->mVideoExtParams.ui16CropTop)
								{

									sInputBuffer.bEnableCorp = 1;
									sInputBuffer.sCropInfo.nLeft = pSelf->mVideoExtParams.ui16CropLeft;
									sInputBuffer.sCropInfo.nWidth = pSelf->m_sOutPortDefType.format.video.nFrameWidth
																	- pSelf->mVideoExtParams.ui16CropLeft
																	- pSelf->mVideoExtParams.ui16CropRight;
									sInputBuffer.sCropInfo.nTop = pSelf->mVideoExtParams.ui16CropTop;
									sInputBuffer.sCropInfo.nHeight = pSelf->m_sOutPortDefType.format.video.nFrameHeight;
						...
						else
						{
							...
							else
							{
								...
								if(pSelf->m_sInPortFormatType.eColorFormat == OMX_COLOR_FormatAndroidOpaque)
								{
									...
									//* get the argb buffer.
									bufferHanle = *(buffer_handle_t*)(pInBufHdr->pBuffer+pInBufHdr->nOffset + 4);
			                    	android::Rect rect((int)pSelf->m_sInPortDefType.format.video.nStride, (int)pSelf->m_sInPortDefType.format.video.nFrameHeight);
        ...
        if (FD_ISSET(pSelf->m_venc_cmdpipe[0], &rfds))
        {
			...
			switch(cmd)
			{
				...
				case OMX_Venc_Cmd_ChangeBitrate:
					logd("pSelf->m_framerate: %d, bitrate: %d", (int)pSelf->m_framerate,(int) pSelf->m_sOutPortDefType.format.video.nBitrate);
					VideoEncSetParameter(pSelf->m_encoder, VENC_IndexParamBitrate, &pSelf->m_sOutPortDefType.format.video.nBitrate);
				...
				case OMX_Venc_Cmd_ChangeFramerate:
					logd("pSelf->m_framerate: %d, bitrate: %d", (int)pSelf->m_framerate, (int)pSelf->m_sOutPortDefType.format.video.nBitrate);
					VideoEncSetParameter(pSelf->m_encoder, VENC_IndexParamFramerate, &pSelf->m_framerate);
					setSuperFrameCfg(pSelf);
					setSVCSkipCfg(pSelf);
					break;
				default:
